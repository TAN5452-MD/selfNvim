...
function M.setFTToggleMap(key, name, cmd)
  G.map({
    { 'n', key, string.format(":lua require('vim-floaterm').toggleFT('%s', '%s')<cr>", name, cmd),
      {
        silent = true,
        noremap = true
      } },
...
function M.config()
  ...
  -- G.cmd("hi FloatermBorder ctermfg=07 ctermbg=none")

  M.setFTToggleMap('<c-t>', 'TERM', '')
  M.setFTToggleMap('<c-f>', 'RANGER', 'ranger')
  M.setFTToggleMap('<c-b>', 'DBUI', 'nvim +CALLDB')
  G.map({
    { 'n', '<F5>', ':lua require("vim-floaterm").runFile()<cr>', {
      silent = true,
      noremap = true
    } },
    { 'i', '<F5>', '<esc>:lua require("vim-floaterm").runFile()<cr>', {
      silent = true,
      noremap = true
    } },
    { 't', '<F5>',
